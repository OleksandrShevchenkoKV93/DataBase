...
class View:
    ...
    def print_orders(self, table):
        print('Order table:')
        print("%10s%20s%10s%10s%12s" % ("order_id", "customer_id", "car_id", "stuff_id", "order_date"))
        for row in table:
            print(row)

    def print_customers(self, table):
        print('Customers table:')
        print("%10s%25s%10s%25s" % ("customer_id", "customer_name", "customer_surname", "customer_telephone"))
        for row in table:
            print(row)

    def print_cars(self, table):
        print('Cars table:')
        print("%10s%60s%60s%25s" % ("car_id", "brand_name", "model_name", "year_of_manufacture"))
        for row in table:
            print(row)

    def print_stuff(self, table):
        print('Stuff table:')
        print("%10s%35s%15s" % ("stuff_id", "stuff_surname", "stuff_telephone","date_start_working"))
        for row in table:
            print(row)
